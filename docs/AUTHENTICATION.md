...
2. `GET /api/auth/logout` is called to ensure server-side cleanup
...
2. A 1-hour token is generated and emailed to the user's registered address
...
`POST /api/auth/2fa/regenerate-backup-codes` with `{ password, code }`

- Same authentication requirements as viewing
...
- Falls back to a **42-entry hardcoded fallback** list if the GitHub fetches fail
