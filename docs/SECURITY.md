...
| #9 | Stale fetch race conditions | List pages use AbortController to cancel in-flight requests when new requests are made, preventing stale data from overwriting fresh data | `src/app/(dashboard)/projects/page.tsx`, `src/app/(dashboard)/projects/[id]/page.tsx` |
